#!/usr/bin/env bash
# Install pack-creator from GitHub Releases (CntierTeam/pack-creator).
#
# Usage:
...
#
# Env:
#   REPO         default CntierTeam/pack-creator
...
case "$os-$arch" in
  linux-x86_64|linux-amd64)
    ...
    ASSET="${ASSET:-pack-creator-linux-x86_64-musl}"
    FALLBACK_ASSET="pack-creator-linux-x86_64-gnu"
    OUT_NAME="$BIN_NAME"
    ;;
  ...
  mingw*|msys*|cygwin*|windows*)
    ASSET="${ASSET:-pack-creator-windows-x86_64.exe}"
    FALLBACK_ASSET=""
