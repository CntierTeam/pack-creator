#!/usr/bin/env bash
# Install the pack-creator Codex skill into $CODEX_HOME/skills (default ~/.codex/skills).
#
# Modes:
#   copy|link     use local skills/pack-creator from this checkout (dev)
#   release       download skill sources from GitHub (CntierTeam/pack-creator)
#
# Usage:
#   ./scripts/install-codex-skill.sh              # copy
#   ./scripts/install-codex-skill.sh link
#   ./scripts/install-codex-skill.sh release       # latest main/master archive
#   VERSION=v0.1.0 ./scripts/install-codex-skill.sh release
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SRC="$REPO_ROOT/skills/pack-creator"
CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"
DEST_ROOT="${CODEX_HOME}/skills"
DEST="$DEST_ROOT/pack-creator"
MODE="${1:-copy}"
REPO="${REPO:-CntierTeam/pack-creator}"
...
case "$MODE" in
  ...
  release)
    ...
    skill_src="$root/skills/pack-creator"
    [[ -f "$skill_src/SKILL.md" ]] || { echo "error: skills/pack-creator not in archive" >&2; exit 1; }
